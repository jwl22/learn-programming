...
class ViewController: UIViewController {
    ...
    @IBAction func btnLampRemove(_ sender: UIButton) {
        ...
        let offAction = UIAlertAction(title: "아니오, 끕니다(off).", style: UIAlertAction.Style.default, handler: {ACTION in self.lamping.image = self.imgOff
                                      self.isLampOn = false})
        let onAction = UIAlertAction(title: "아니오, 켭니다(on).", style: UIAlertAction.Style.default, handler: {ACTION in self.lamping.image = self.imgOn
            self.isLampOn = true})
